 ...
 * {@link com.google.appengine.api.search.checkers.SearchApiLimits#MAXIMUM_TEXT_LENGTH}. Atoms as
 ...
 * equality. For example, if you add a field with name {@code code} and an ATOM value of "928A
 * 33B-1", then query {@code code:"928A 33B-1"} would match the document with this field, while
 * query {@code code:928A} would not. TEXT fields, unlike ATOM, match both on equality or if any
 * token extracted from the original field matches. Thus if {@code code} field had the value set
 * with {@link Field.Builder#setText(String)} method, both queries would match. Finally, HTML fields
