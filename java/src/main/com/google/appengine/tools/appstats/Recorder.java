...
public class Recorder implements Delegate {
  ...
  public interface RecordWriter {
    ...
    /**
     * Notifies the writer that a batch of RPCs has been ended. The writer may use this information
     * to write cached records to the data store.
     *
     * @param wrappedDelegate the delegate that this recorder wraps (enabling the RecordWriter to
     *        bypass the recorder's interception of rpcs)
     * @param environment the current environment.
     * @param httpResponseCode the http response code to be persisted.
  ...
  static void createStackTrace(int numLinesToIgnore, IndividualRpcStatsProto.Builder stats,
      int maxNumLines) {
   ...
   * @param settings the appstats settings for this recorder.
   */
  public Recorder(Delegate<?> wrapThis, RecordWriter writer, AppstatsSettings settings) {
   ...
   * @param settings the appstats settings for this recorder.
   */
  Recorder(Delegate<?> wrapThis, RecordWriter writer, Clock clock, AppstatsSettings settings) {
  ...
  int getMaxLinesOfStackTrace() {
    return settings.getMaxLinesOfStackTrace();
  }

  PayloadRenderer getPayloadRenderer() {
    return settings.getPayloadRenderer();
  }
  ...
  public Future<byte[]> makeAsyncCall(Environment environment,
    ...
    RecordingData intermediary = new RecordingData(packageName, methodName,
        settings.isCalculateRpcCosts(), settings.isDatastoreDetails());
  ...
  public byte[] makeSyncCall(
    ...
    intermediary.storeResultData(settings.getPayloadRenderer(), request);
  ...
  private void processRecordingFuture(RecordingFuture current, Environment environment) {
    ...
    if (current.isDone() || current.isCancelled()) {
      ...
      recordingData.storeResultData(settings.getPayloadRenderer(), current.getRequest());
      ...
      try {
        settings.getUnprocessedFutureStrategy().onUnprocessedFuture(current);
      } catch (InterruptedException e) {
      } catch (ExecutionException e) {
      }
    }
