...
class RecordingData {
  ...
  void recordPutDetails() {
    ...
    DatastoreV3Pb.PutResponse resultResponse = new DatastoreV3Pb.PutResponse();
    ...
    for (OnestoreEntity.Reference key : resultResponse.keys()) {
      details.addKeysWritten(bridge.convert(key));
    }
  }
  ...
  void recordGetDetails(byte[] request) {
    ...
    DatastoreV3Pb.GetResponse resultResponse = new DatastoreV3Pb.GetResponse();
    resultResponse.mergeFrom(response);
    ProtoConverter<OnestoreEntity.Reference, Reference> bridge = protoConverter();

    for (OnestoreEntity.Reference key : getRequest.keys()) {
      details.addKeysRead(bridge.convert(key));
    }

    for (DatastoreV3Pb.GetResponse.Entity entity : resultResponse.entitys()) {
  ...
  void recordQueryDetails(byte[] request) {
    ...
    if (methodName.equals("RunQuery")) {
      ...
      if (queryRequest.hasAncestor()) {
        details.setQueryAncestor(bridge.convert(queryRequest.getAncestor()));
      }
