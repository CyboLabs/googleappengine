...
public class AppYamlTranslator {
  ...
  private void translateAppEngineWebXml(StringBuilder builder) {
    ...
    if (appEngineWebXml.getUseVm()) {
      ...
      appendVmSettings(appEngineWebXml.getVmSettings(), builder);
      appendVmHealthCheck(appEngineWebXml.getVmHealthCheck(), builder);
    }
  }

  /**
   * Appends the given VM Settings as YAML to the given StringBuilder.
   *
   * @param vmSettings The vm settings map to append as YAML.
   ...
   * @param builder The StringBuilder to append to.
   */
  private void appendVmSettings(Map<String, String> vmSettings, StringBuilder builder) {
    builder.append("vm_settings:\n");
    if (vmSettings == null || !vmSettings.containsKey("image")) {
      if (sdkVersion != null && sdkVersion.getRelease() != null) {
        builder.append("  'image': " + yamlQuote(sdkVersion.getRelease()) + "\n");
      }
    }
    if (vmSettings != null) {
      for (Map.Entry<String, String> setting : vmSettings.entrySet()) {
        builder.append(
  ...
  private void appendVmHealthCheck(VmHealthCheck vmHealthCheck, StringBuilder builder) {
    builder.append("vm_health_check:\n");
    if (vmHealthCheck.getEnableHealthCheck()) {
      builder.append("  enable_health_check: True\n");
    } else {
      builder.append("  enable_health_check: False\n");
    }

    appendIfNotNull(builder, "  check_interval_sec: ", vmHealthCheck.getCheckIntervalSec());
    appendIfNotNull(builder, "  timeout_sec: ", vmHealthCheck.getTimeoutSec());
    appendIfNotNull(builder, "  unhealthy_threshold: ", vmHealthCheck.getUnhealthyThreshold());
    appendIfNotNull(builder, "  healthy_threshold: ", vmHealthCheck.getHealthyThreshold());
    appendIfNotNull(builder, "  restart_threshold: ", vmHealthCheck.getRestartThreshold());
    appendIfNotNull(builder, "  host: ", vmHealthCheck.getHost());
  }
      ...
      if (fallthrough) {
        ...
        if (hasJsps) {
          intersector.addGlob(GlobFactory.createGlob(
              "*.jsp",
              DYNAMIC_PROPERTY, true));
        } else if (appEngineWebXml.getUseVm()) {
          intersector.addGlob(GlobFactory.createGlob(
              "*.jsp",
              DYNAMIC_PROPERTY, true));
        }
